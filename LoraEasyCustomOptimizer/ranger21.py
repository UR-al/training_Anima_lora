...
# Source: https://github.com/kozistr/pytorch_optimizer/blob/main/pytorch_optimizer/optimizer/ranger21.py
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if p.dtype in {torch.float16, torch.bfloat16}:
                    copy_stochastic_(state['variance_ma'], variance_ma)          
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # norm loss
                correction = 2.0 * self.norm_loss_factor * (1.0 - 1.0 / unit_norm(p_fp32).add_(group['eps']))
                ...
                if p.dtype in {torch.float16, torch.bfloat16}:
                    grad_ma = grad_ma.to(torch.float32)
                    neg_grad_ma = neg_grad_ma.to(torch.float32)
                    variance_ma = variance_ma.to(torch.float32)
                    max_variance_ma = max_variance_ma.to(torch.float32)

                torch.max(state['max_variance_ma'], variance_ma, out=state['max_variance_ma'])

                de_nom = (state['max_variance_ma'].sqrt() / bias_correction2_sq).add_(group['eps'])
                ...
                pn_momentum = grad_ma.mul(1.0 + self.beta0).add_(neg_grad_ma, alpha=-self.beta0).mul_(1.0 / noise_norm)
                p_fp32.addcdiv_(pn_momentum, de_nom, value=-step_size)

                if p.dtype in {torch.float16, torch.bfloat16}:
                    copy_stochastic_(p, p_fp32)
                    if group['step'] % 2 == 1:
                        copy_stochastic_(state['grad_ma'], grad_ma)
                    else:
                        copy_stochastic_(state['neg_grad_ma'], grad_ma)
