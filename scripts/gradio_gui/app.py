...
def build_app(default_port: int = 7860):
    ...
    with gr.Blocks(title="Anima LoRA Trainer") as demo:
        ...
        with gr.Tab("LoRA"):
            ...
            with gr.Row():
                reg("method", gr.Dropdown(
                    methods, value=(methods[0] if methods else "lora"),
                    label="Method (configs/methods/<name>.toml)",
                ))
                reg("preset", gr.Dropdown(
                    presets, value=("default" if "default" in presets else (presets[0] if presets else "default")),
            ...
            with gr.Accordion("Output", open=True):
                with gr.Row():
                    reg("output_name", gr.Textbox(
                        label="Output name", placeholder="(defaults to method name)",
                    ))
                    reg("output_dir", gr.Textbox(
                        value="output", label="Output base dir",
                    ))

            # ── Anima model paths (mirrors kohya's class_anima accordion) ────
            with gr.Accordion("Anima Model Paths (blank = config-chain default)", open=False):
                reg("dit_path", gr.Textbox(
                    label="DiT checkpoint (--pretrained_model_name_or_path)",
                    placeholder="Path to the Anima DiT .safetensors",
                ))
                reg("te_path", gr.Textbox(
                    label="Qwen3 text encoder (--qwen3)",
                    placeholder="Path to Qwen3-0.6B model dir / .safetensors",
                ))
                reg("vae_path", gr.Textbox(
                    label="VAE (--vae)",
            ...
            with gr.Accordion("Basic", open=True):
                with gr.Row():
                    reg("learning_rate", gr.Textbox(label="Learning rate", placeholder="(method default)"))
                    reg("max_train_epochs", gr.Textbox(label="Max train epochs", placeholder="(method default)"))
                    reg("seed", gr.Textbox(label="Seed", placeholder="(random)"))
                with gr.Row():
                    reg("network_dim", gr.Textbox(label="Network dim / rank", placeholder="(method default)"))
                    reg("network_alpha", gr.Textbox(label="Network alpha", placeholder="(method default)"))

            # ── Optimizer & scheduler ───────────────────────────────────────
            with gr.Accordion("Optimizer & Scheduler", open=False):
                with gr.Row():
                    reg("optimizer_type", gr.Dropdown(
                        optimizers, value=(optimizers[0] if optimizers else "AdamW"),
                        label="Optimizer (kohya built-ins + vendored zoo)", allow_custom_value=True,
                    ))
                    reg("lr_scheduler_type", gr.Dropdown(
                        schedulers, value="", label="LR scheduler (blank = config default)",
                        allow_custom_value=True,
                    ))
                with gr.Row():
                    reg("lr_warmup_steps", gr.Textbox(label="LR warmup steps", placeholder="(default)"))
                    reg("optimizer_args", gr.Textbox(
                        label="optimizer_args (k=v …)", placeholder="weight_decay=0.01 betas=0.9,0.99",
                    ))
                    reg("lr_scheduler_args", gr.Textbox(
                        label="lr_scheduler_args (k=v …)", placeholder="",
                    ))

            # ── Network / adapter ───────────────────────────────────────────
            with gr.Accordion("Network / Adapter", open=False):
                reg("network_module", gr.Dropdown(
                    network_modules, value="", label="network_module (blank = method default)",
                    allow_custom_value=True,
                ))
                reg("network_args", gr.Textbox(
                    label="network_args (k=v …)", placeholder="conv_dim=8 conv_alpha=4",
                ))

            # ── Dataset ─────────────────────────────────────────────────────
            with gr.Accordion("Dataset", open=False):
                reg("dataset_config", gr.Textbox(
            ...
            with gr.Accordion("Monitor & Run", open=True):
                with gr.Row():
                    reg("daemon", gr.Checkbox(value=True, label="Run via daemon (detached; survives close)"))
                    reg("monitor", gr.Checkbox(value=False, label="Web loss monitor (--monitor)"))
                with gr.Row():
                    reg("monitor_host", gr.Textbox(value="127.0.0.1", label="Monitor host"))
                    reg("monitor_port", gr.Textbox(label="Monitor port", placeholder="8766"))
                    reg("log_every_n_steps", gr.Textbox(label="Log every N steps", placeholder="(default)"))

            with gr.Accordion("Extra CLI flags", open=False):
                reg("extra_flags", gr.Textbox(
