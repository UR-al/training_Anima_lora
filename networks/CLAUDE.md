...
| `lycoris_anima.py` | Bridge to stock `lycoris-lora` (LoKr/LoHa/DyLoRA/GLoRA/Full/Diag-OFT/BOFT/IA3) for the Anima DiT — select via `network_module = "networks.lycoris_anima"`. Sanitizes Anima's cached-TE `[None]` slot (which crashes stock `lycoris.kohya`), then delegates verbatim. Targeting comes from `configs/lycoris_presets/anima_attn_mlp.toml` (197 modules: attn+MLP) / `anima_full.toml` (314: +adaln/embeds) via `network_args preset=…` — stock LyCORIS presets list diffusers class names absent from the Anima blocks. Speed core (torch.compile) survives; ride the clean `configs/methods/lycoris.toml`. |
