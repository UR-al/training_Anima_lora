...
# Ported from sd-scripts (kohya-ss) commit 2a81265 "feat(anima): add image-only 2D
# Qwen-Image VAE option" — imports rewired to this fork's module layout. The 2D blocks
# are self-contained; only these shared pieces come from the 3D VAE module.
...
def load_vae(
    ...
    Drop-in for ``library.models.qwen_vae.load_vae`` — same signature (incl. ``dtype`` /
    ``eval`` conveniences and repo-home path resolution) so the dispatcher can pick 2D vs
    3D transparently. ``disable_cache`` is accepted for signature parity only (the 2D VAE
    has no temporal cache, so it is ignored).
    """
