...
def _custom_optimizer_registry() -> dict:
    """The vendored ``LoraEasyCustomOptimizer`` name->class registry (or empty).

    Imported lazily and guarded so the trainer still works if the optimizer zoo
    isn't vendored. The registry is keyed by ``classname.lower()`` and only holds
    optimizers actually loadable on the current stack (the package is itself
    import-guarded), so a name present here is guaranteed constructible.
    """
    try:
...
def _resolve_custom_optimizer_path(name: str):
    """Friendly optimizer name -> ``'module.Qualname'`` dotted path, or ``None``."""
    cls = _custom_optimizer_registry().get(name.lower())
    if cls is None:
        return None
    return f"{cls.__module__}.{cls.__qualname__}"


def _resolve_optimizer_class(name_or_path: str):
    """Resolve a friendly name OR a dotted path to an optimizer class.

    Used for meta-optimizer (wrapper) ``base_optimizer_type`` resolution.
    """
    if "." in name_or_path:
        values = name_or_path.split(".")
        module = importlib.import_module(".".join(values[:-1]))
        return getattr(module, values[-1])
    cls = _custom_optimizer_registry().get(name_or_path.lower())
    if cls is not None:
        return cls
    return getattr(torch.optim, name_or_path)
...
def get_optimizer(args, trainable_params) -> tuple[str, str, object]:
    ...
    # Up-front friendly-name resolution for the vendored optimizer zoo. A bare
    # vendored name (e.g. "ProdigyPlusScheduleFree") is mapped to its dotted path
    # here so the kohya built-in branches below — which match on suffixes like
    # "schedulefree" and would eagerly import optional deps (schedulefree) — don't
    # capture it; it goes straight to the dotted-path resolver instead.
    custom_dotted = (
        _resolve_custom_optimizer_path(optimizer_type)
    ...
    if args.optimizer_args is not None and len(args.optimizer_args) > 0:
        for arg in args.optimizer_args:
            ...
            try:
                ...
                # Bare-string value (e.g. update_strategy=cautious) — not a Python
                # literal, so keep it as the raw string (LoRA_Easy parity: numbers/
                # tuples/bools still parse, words stay words).
                pass
    ...
    elif custom_dotted is None and optimizer_type.endswith("schedulefree".lower()):
        try:
    ...
    if optimizer is None:
        # Friendly-name rewrite for the vendored optimizer zoo: a bare name like
        # "CAME" was mapped to its fully-qualified dotted path up-front (custom_dotted),
        # so users don't have to spell out "LoraEasyCustomOptimizer.came.CAME".
        case_sensitive_optimizer_type = (
            custom_dotted if custom_dotted is not None else args.optimizer_type
        )
        ...
        # Meta-optimizer (wrapper) support: optimizers such as ScheduleFreeWrapper
        # / SODAWrapper take a *base optimizer class* as their first non-param
        # argument. The user selects it via `base_optimizer_type=<name-or-path>`
        # in optimizer_args; resolve it (friendly name or dotted path) to a class.
        base_optimizer_type = optimizer_kwargs.pop("base_optimizer_type", None)
        if base_optimizer_type is None:
            base_optimizer_type = optimizer_kwargs.pop("base_optimizer", None)
        if base_optimizer_type is not None:
            base_optimizer_class = _resolve_optimizer_class(base_optimizer_type)
            logger.info(
                f"  wrapper base optimizer: "
                f"{base_optimizer_class.__module__}.{base_optimizer_class.__name__}"
            )
            optimizer = optimizer_class(
                trainable_params, base_optimizer_class, lr=lr, **optimizer_kwargs
            )
        else:
            optimizer = optimizer_class(trainable_params, lr=lr, **optimizer_kwargs)
...
def is_schedulefree_optimizer(optimizer: Optimizer, args: argparse.Namespace) -> bool:
    optimizer_type = getattr(args, "optimizer_type", None) or ""
    if optimizer_type.lower().endswith("schedulefree".lower()):
        return True
    # Capability-based fallback: many grafted schedule-free / wrapper optimizers
    # (e.g. ScheduleFreeWrapper, AdamWScheduleFreePlus) manage their own LR and
    # expose train()/eval() but don't end in "schedulefree". Plain torch / kohya
    # optimizers have neither method, so this stays a safe heuristic.
