...
## Optimizers, schedulers & a live web monitor

This build merges the broad optimizer/scheduler support of LoRA_Easy_Training_Scripts and the live dashboard of AnimaLoraToolkit onto the fast trainer.

- **~89 optimizers, by name.** `--optimizer_type CAME` (or `ADOPT`, `Prodigy`, `ProdigyPlusScheduleFree`, `FMARSCrop`, `OCGOpt`, … or any `pkg.module.Class`). The kohya built-ins (`AdamW` fused, `AdamW8bit`, `DAdapt*`, `Adafactor`, `*ScheduleFree`) still work. Meta-optimizers wrap a base: `--optimizer_args base_optimizer_type='CAME'`. Missing optional deps are skipped, never fatal.
- **Custom LR schedulers.** `--lr_scheduler_type LoraEasyCustomOptimizer.RexAnnealingWarmRestarts.RexAnnealingWarmRestarts --lr_scheduler_args gamma=0.9 first_cycle_max_steps=... --lr_warmup_steps 0` (the scheduler does its own warmup). `warmup_stable_decay`, `cosine_with_min_lr`, etc. are built in.
- **Live web monitor.** Add `--monitor` (or `monitor = true` in any config) to serve a Chart.js loss/LR/sample dashboard at `http://127.0.0.1:8765` (`--monitor_host`/`--monitor_port`/`--monitor_open_browser`). It attaches at the existing per-step hook (no overhead, can't crash training), rehydrates the curve on `--resume`, and shows sample previews. Pure stdlib — no extra deps.

```bash
python tasks.py lora --optimizer_type CAME --monitor
```
