...
| `gui/kohya/` | Gradio control panel (kohya_ss-style layout) — `python tasks.py gradio-gui`. A thin UI re-skin over `gui/webgui/server.py`, so it drives **this repo's** `train.py`, not kohya `sd-scripts`. Includes a live training-log panel that tails the subprocess's captured logfile (the sd-scripts RichHandler console) + a **Config file load/save** panel. Opt-in `gradio` extra (`uv sync --extra gradio`). (Moved from `scripts/gradio_gui/`.) |
| `gui/modules/config_io.py` | TomlFunctions-equiv: `load_toml_to_form` (LETS/kohya/anima_lora `--config_file` TOML → GUI form, applying key renames — `timestep_sample_method`→`timestep_sampling`, `cache_latents`→`use_vae_cache`, `min/max_timestep`÷1000→`t_min/t_max` — and folding unmapped keys into Extra CLI flags) + `save_form_to_toml` (form → runnable config via the server's real arg builder). Torch-free; covered by `tests/test_config_io.py`. |
...
Subsets accept `cache_dir` — redirects all VAE/TE/PE caches to that dir with stem-mirrored names, **split by type into `vae/` · `te/` · `pe/` subfolders** (derived from the cache suffix by `library.io.cache.resolve_cache_path::cache_subdir_for`; the pooled sidecar rides `te/`). Write and read both flow through `resolve_cache_path`, so the split is symmetric; a legacy flat cache (pre-split, no subfolder) is still located via a read-fallback. EasyControl uses `cache_dir` to keep source dirs user-facing while caches live under `post_image_dataset/`. `library.config.io.load_method_preset(method, preset, methods_subdir=...)` is the reusable merge helper (not re-exported via `train_util`). All config paths are relative to `anima_lora/`. Outputs split by kind: checkpoints (+ `.snapshot.toml` + `_moe` siblings) in `output/ckpt/`, inference images in `output/tests/`.

## Merged capabilities — optimizer zoo / custom schedulers / web monitor

This fork merges two donors onto the anima_lora base (see also `library/monitoring/` and the vendored `custom_scheduler/LoraEasyCustomOptimizer/` package — a second install source root, still imported as top-level `LoraEasyCustomOptimizer`):

- **Optimizer zoo (~89, from LoRA_Easy_Training_Scripts).** `--optimizer_type <name>` accepts a *friendly name* (`CAME`, `ADOPT`, `Prodigy`, `ProdigyPlusScheduleFree`, `FMARSCrop`, `OCGOpt`, …) — `library/training/optimizers.py` resolves it through the vendored `LoraEasyCustomOptimizer` registry, then falls through to the kohya dotted-path `importlib` loader, so a fully-qualified `pkg.module.Class` also works. The kohya built-ins (`AdamW` fused, `AdamW8bit`, `DAdapt*`, `Prodigy`, `Adafactor`, `*ScheduleFree`) are unchanged. **Meta-optimizers** (`ScheduleFreeWrapper`, `SODAWrapper`) take a base via `--optimizer_args base_optimizer_type='CAME'`. The package's `__init__.py` is **import-guarded** + installs a torchao/bitsandbytes compat shim (`_install_compat_shims`: re-adds `TORCH_VERSION_AT_LEAST_2_4/5/6`, sets `BNB_CUDA_VERSION=130` on CUDA 13.x), so optimizers whose optional deps are missing are skipped (`L.available_optimizers()` / `L.skipped_optimizers()`), never breaking import. Known non-working in this trainer (niche; use the many alternatives): torchao AO 8/4-bit (require a tensor `lr`) and `AdamWScheduleFreePlus` (needs a loss closure threaded into the shared step path). `SCION`'s upstream `pbar_hat` scope bug is patched in the vendored copy.
- **Custom schedulers (CAWR/RAWR, from LoRA_Easy).** `--lr_scheduler_type <dotted path>` resolves any LR scheduler via `library/training/schedulers.py` (e.g. `LoraEasyCustomOptimizer.CosineAnnealingWarmRestarts.CosineAnnealingWarmRestarts`), with `--lr_scheduler_args key=value …`. The scheduler usually handles its own warmup, so pass `--lr_warmup_steps 0`. `warmup_stable_decay` / `cosine_with_min_lr` etc. are already built in. Schedule-free optimizers auto-bypass the scheduler (capability-detected).
- **Web monitor (from AnimaLoraToolkit).** `--monitor` (or `monitor = true` in any config TOML — the flags auto-register in the config schema) starts a pure-stdlib Chart.js loss/LR/sample dashboard at `http://<monitor_host>:<monitor_port>` (`--monitor_host` default `127.0.0.1`, `--monitor_port` 8765, `--monitor_open_browser`). It attaches as a composing `ProgressSink` (`library/monitoring/sink.py::MonitorSink`) at the existing per-step hook — **no hot-loop edits** — so it costs ~nothing and can't crash training (all emits guarded). On resume (`--resume`/`--network_weights`) the loss curve rehydrates from `library/monitoring/monitor_data/state.json`; freshly-decoded sample PNGs (`<output_dir>/sample/`) surface in the dashboard. Set `monitor_port` if it collides with another local service.
...
| **Turbo** | DP-DMD (diversity-preserved DMD) distillation; output is a normal LoRA. | Bespoke schema read by `scripts/distill_turbo/` — don't `print-config`. Bespoke two-optimizer loop (student + fake/critic) kept out of `train.py`; converges only the leaves — writes a canonical `output/ckpt/<name>.snapshot.toml`. `docs/experimental/dpdmd.md` (ops), `docs/structure/dpdmd.md` (structure); CA-era history in `docs/proposal/dmd2_decoupled_improvements.md`. |
